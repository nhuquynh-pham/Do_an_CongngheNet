...
namespace Do_an_CongngheNET
{
    public partial class Quanlithongke : Form
    {
        ...
        {
            // ================================================================
            // KIỂM TRA QUYỀN TRUY CẬP — nhất quán với Chuyenphong
            // ================================================================
            if (!SessionManager.CoQuyen("CN010"))
            {
            ...
            try
            {
                // ── Gán Tag cho nút — giống Chuyenphong ──────────────────
                // "select"      = hiển thị khi đang xem (chưa nhập)
                // "confirm"     = hiển thị khi đang nhập/sửa
                // "AlwaysEnable"= luôn bật (các ô lọc)
                btnThongke.Tag = "select";
                tbnLammoi.Tag = "select";
                tbnInketqua.Tag = "select";
                tbnXemchitiet.Tag = "select";
                tbnXuatEx.Tag = "select";
                btnKetthuc.Tag = "select";   // nút đóng luôn ở nhóm "select"
                ...
                // ── Trạng thái ban đầu: chỉ xem — giống Chuyenphong ─────
                UIService.SetInputsEnabled(this, false);
                UIService.SetButtonsEnabled(this, false);

                // Luôn cho phép tìm kiếm — giống Chuyenphong
        ...
        {
            if (dgvChuyenphong_CurrentRow() == null)
            {
        ...
        private void LoadKhuNha()
        {
            ...
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                    cbokhunha.Items.Add(row["TenKhu"].ToString().Trim());

                cbokhunha.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Lỗi LoadKhuNha",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void ThongKeTongQuat(int? nam, int? thang)
        {
            ...
            object dtObj = _db.ExecuteScalar(
                BuildDoanhThuSql(null, null, nam, thang),
                BuildParams(null, null, nam, thang));
            long tongDT = dtObj == null || dtObj == DBNull.Value ? 0 : Convert.ToInt64(dtObj);

            object dnObj = _db.ExecuteScalar(
                BuildDienNuocSql(null, null, nam, thang),
                BuildParams(null, null, nam, thang));
        ...
        // ── Helper: trả về CurrentRow của dgvThongKe (tránh typo cũ) ────
        private DataGridViewRow dgvChuyenphong_CurrentRow()
            => dgvThongKe.CurrentRow;
